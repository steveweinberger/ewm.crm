...
COG.Library.Case = {
    ...
    FormOnLoad: function () {
        ...
        COG.Library.Case.SetCustomerIdDefault();
    ...
    SetCustomerIdDefault: function () {
        var caseid = Xrm.Page.data.entity.getId();
        if (caseid == null)
            return;

        var contactQuery = "<fetch><entity name='contact'><attribute name='contactid' /><filter type='and'><condition attribute='fullname' operator='eq' value='System User' /></filter></entity></fetch>";            
        var contacts = XrmServiceToolkit.Soap.Fetch(contactQuery);
        var contactId = contacts[0].attributes.contactid.value.toString();

        var caseContactQuery = "<fetch><entity name='incident'><attribute name='customerid' /><filter type='and'><condition attribute='customerid' operator='eq' uiname='System User' uitype='contact' value='{73B95E69-D7CE-E711-A82C-000D3A37C848}' /><condition attribute='incidentid' operator='eq' uitype='incident' value='" + caseid + "' /></filter></entity></fetch>";
        var cases = XrmServiceToolkit.Soap.Fetch(caseContactQuery);

        if (cases.length != 1) {
            COG.Library.Case.SetLookUp("customerid", "contact", contactId, "System User");
        }
        //Set Business Process Find Customer Field
        //Phone to Case Process
        //Xrm.Page.ui.process != null && Xrm.Page.ui.process.setDisplayState("collapsed")
        //var findCustomer = Xrm.Page.ui.process.getAttribute("Find Customer").getValue();
            
        //COG.Library.Case.SetLookUp("Find Customer", "contact", findCustomerId, "System User");
            
        //}
        //else if(Xrm.Page.data.entity.attributes.get('customerid').getValue() != "System User")
        //{
        //    alert('into else if');
        ...
        //var findCustomerId = Xrm.Page.ui.process.attributes.get('Find Customer').getValue()[0].id;
        //var findCustomerName = Xrm.Page.ui.process.attributes.get('Find Customer').getValue()[0].name;
        //(fieldName, fieldType, fieldId, value)
        //COG.Library.Case.SetLookUp("customerid", "contact", customerId, "System User");
